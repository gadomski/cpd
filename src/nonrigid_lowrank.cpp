...
NonrigidLowrank::NonrigidLowrank(float tol, int max_it, float outliers,
                                 bool use_fgt, float epsilon, float beta,
                                 float lambda, arma::uword numeig)
...
Registration::ResultPtr NonrigidLowrank::execute(const arma::mat& X,
    ...
    DEBUG("parameters, tol: " << get_tol() << ", max_it: " << get_max_it()
                              << ", outliers: " << get_outliers()
                              << ", epsilon: " << get_epsilon() << ", beta: "
                              << get_beta() << ", lambda: " << get_lambda());

    const arma::uword numeig =
        (get_numeig() == 0) ? arma::uword(std::sqrt(Y.n_rows)) : get_numeig();
    ...
    while (iter < get_max_it() && ntol > get_tol() &&
        ...
        DEBUG("nonrigid_lowrank iteration: dL= " << ntol << ", iter= " << iter
                                                 << ", sigma2= " << sigma2);
        ...
        W = 1 / (get_lambda() * sigma2) *
            (F -
             dPQ * (arma::solve(get_lambda() * sigma2 * invS + Q.t() * dPQ,
                                Q.t() * F)));
